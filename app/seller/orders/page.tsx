...
export default function SellerOrdersPage() {
  ...
  useEffect(() => {
    // Redirect to seller-specific orders management
    router.replace('/seller/orders/management');
  }, [router]);

  const loadSellerOrders = (sellerEmail: string) => {
    try {
      // Get all orders from localStorage
      const allOrders = JSON.parse(localStorage.getItem('user_orders') || '[]');

      // Get all products to match seller
      ...
      const sellerOrders = allOrders.filter((order: Order) => {
        return order.items.some((item: OrderItem) => {
          ...
          return product && (product.seller_email === sellerEmail || product.seller_id === sellerEmail);
        });
      }).map((order: Order) => {
      ...
      sellerOrders.forEach((order: Order) => {
  ...
  const updateOrderStatus = (orderId: string, newStatus: string) => {
    try {
      ...
      // Update local state
      const updatedLocalOrders = orders.map((order: any) => 
        order.id === orderId 
          ? { ...order, status: newStatus, delivery_date: newStatus === 'delivered' ? new Date().toISOString() : order.delivery_date }
          : order
      );
  ...
  const updateTrackingId = (orderId: string) => {
    ...
    try {
      ...
      // Update local state
      const updatedLocalOrders = orders.map((order: any) => 
        order.id === orderId ? { ...order, tracking_id: trackingId.trim() } : order
      );
      setOrders(updatedLocalOrders);
              ...
              <div className="sm:w-48">
                <Select value={filterStatus} onValueChange={setFilterStatus}>
                  ...
                  <SelectContent className="bg-white border border-gray-200 shadow-lg">
                    <SelectItem value="all" className="hover:bg-gray-50 focus:bg-gray-50 text-gray-900">All Statuses</SelectItem>
