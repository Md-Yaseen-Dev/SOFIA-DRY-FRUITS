...
import { apiProducts, products as legacyProducts } from '@/lib/mock-data';
  ...
  useEffect(() => {
    const performSearch = async () => {
      ...
      try {
        ...
        const legacyResults = legacyProducts.filter(product => {
          // Text search
          if (query.trim()) {
            const matchesQuery = product.name.toLowerCase().includes(query.toLowerCase()) ||
                                (product.description && product.description.toLowerCase().includes(query.toLowerCase()));
...
export function useQuickSearch(query: string, limit: number = 5) {
  ...
  useEffect(() => {
    const getSuggestions = async () => {
      ...
      const allProducts = [...apiProducts, ...legacyProducts];
